...
indeterminate — missing a phase-sensitive real-form selector, an independent rotation action, and a
Casimir-independent normalisation.
A downstream reconnaissance shows $\overline{\psi_R}\psi_L$ carries Higgs-typed quantum numbers
($H$ or $\widetilde{H}$) in every Yukawa sector, permitting but not generating a composite condensate.
...
representation audit showing that fermionic chiral bilinears carry Higgs-typed quantum numbers (the
doublet $H$ or its conjugate $\widetilde{H}$), without deriving a composite condensate or its
uniqueness.
...
\section[Downstream boundary: chiral transversality and composite channels]{Downstream boundary:
chiral transversality and Higgs-typed composite channels}
...
A separate downstream reconnaissance (Section~\ref{sec:higgsboundary}) shows the same chiral structure
of Eq.~\eqref{eq:transformationlaw} is compatible with, but does not derive, a further role: fermionic
chiral bilinears carry Higgs-typed quantum numbers ($H$ or its conjugate $\widetilde{H}$) in every
audited sector, without determining whether, which, or at what scale a bilinear condenses.
...
The downstream boundary of Section~\ref{sec:higgsboundary} is independent of this soldering question:
it neither uses nor repairs $\rho_{\mathrm{sp}}$, and holds under either reading above.
